...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
     ...
     */
    // Add this visualization method to the RedBlackBST class
    private static class NodeInfo {
        ...
        NodeInfo(String text, int level, int position, boolean isRed) {
            this.text = text;
            this.level = level;
    ...
    // Helper method to get tree information
    private void getTreeInfo(Node node, int level, int position, List<NodeInfo> nodes) {
        if (node == null) return;

        nodes.add(new NodeInfo(node.key.toString() + ":" + node.val.toString(),
                level, position, node.color == RED));

        getTreeInfo(node.left, level + 1, position * 2, nodes);
        getTreeInfo(node.right, level + 1, position * 2 + 1, nodes);
    }

    // Method to print the tree visually
    public void printTree() {
        List<NodeInfo> nodes = new ArrayList<>();
        getTreeInfo(root, 0, 0, nodes);
        ...
        int maxLevel = nodes.stream().mapToInt(node -> node.level).max().orElse(0);
        ...
        for (int level = 0; level <= maxLevel; level++) {
            ...
            List<NodeInfo> levelNodes = nodes.stream()
                    .filter(node -> node.level == finalLevel)
                    .sorted(Comparator.comparingInt(node -> node.position))
                    .collect(Collectors.toList());

            for (int i = 0; i < levelNodes.size(); i++) {
                NodeInfo node = levelNodes.get(i);
                if (node.isRed) {
                    System.out.print("\u001B[31m" + node.text + "\u001B[0m"); // Red color
                } else {
                    System.out.print(node.text); // Black color
                }
    ...
    // Modified main method to show tree structure after each insertion
    public static void main(String[] args) {
        RedBlackBST<String, Integer> st = new RedBlackBST<String, Integer>();

        String[] insertions = {"Y", "L", "P", "M", "X", "H","C","R","A","E", "S"};
        for (int i = 0; i < insertions.length; i++) {
            System.out.println("\nInserting '" + insertions[i] + "':");
            st.put(insertions[i], i);
            st.printTree();
            System.out.println("Size: " + st.size() + ", Height: " + st.height());
            System.out.println("----------------------------------------");
        }
    }
}